...
-----------------------------------------------------------------------------------*/
class WPEC_QV_Uploader extends WPEC_QV_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $post_id = 0 ) {
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'wpecquickview' );
	...
	public function change_button_text( $translation, $original ) {
	    ...
	    if ( $original == 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'wpecquickview' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation =__( 'Use as', 'wpecquickview' ).' '.esc_attr( $_REQUEST['title'] ); }
	    }
	...
	public function modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( 'Gallery', __( 'Previously Uploaded', 'wpecquickview' ), $tabs['gallery'] ); }
		return $tabs;
	}
...
global $wpec_qv_uploader;
$wpec_qv_uploader = new WPEC_QV_Uploader();
